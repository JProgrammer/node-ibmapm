...
if (!process.env.KNJ_LOG_TO_FILE) {
    ...
    log4js.addAppender(log4js.appenders.file('nodejs_restclient.log'), 'knj_jso-sender_log');
}

var logger = log4js.getLogger('knj_jso-sender_log');
...
JsonSender.prototype.register = function register() { // Register DC and Resouce
    ...
    if (process.env.VCAP_APPLICATION) {
        this.vcap = applicationJson = JSON.parse(process.env.VCAP_APPLICATION);
        this.applicationName = this.applicationName || applicationJson.application_name;
        this.nodeAppMD5String = this.vcap.application_id;
    } else {
    ...
    var dcObj = {
        ...
        properties: {
            ...
            displayName: 'NodeJS' + this.applicationName + 'DC',
            version: '0.10.0.1',
            monitoringLevel: 'L1',
                        ...
                        if (typeof service_inst[k] === 'string') {
                            ...
                            for (var c_key in credentials) {
                                if (c_key.indexOf('url') >= 0) {
                                    external_obj.properties[c_key] = credentials[c_key];
                ...
                if (process.env.APM_GW_PROXY_CONNECTION && isHttp && senderTool.httpProxy) {
                    try {
                        senderTool.httpProxy.get('http://' + BM_options.hostname + ':' +
                            BM_options.port + BM_options.path,
