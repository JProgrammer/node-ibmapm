...
JsonSender.prototype.register = function register(){    //Register DC and Resouce
    ...
    if(process.env.VCAP_APPLICATION){
        ...
        var nodeAppMD5 = crypto.createHash('md5');
        ...
        this.nodeAppMD5String = nodeAppMD5.update(this.applicationName +' on '+this.app_hostname).digest('hex');
    }
    ...
    if(process.mainModule.paths && process.mainModule.paths.length > 0){
        ...
        for(var i in process.mainModule.paths){
            var packageFile = process.mainModule.paths[i].split('node_modules')[0]+"/"+"package.json";
            try {
    ...
    try {
        if(envType === 'Cloudnative'){
            ...
            if (process.env.VCAP_APPLICATION) {
                ...
                logger.info('JsonSender: VCAP_APPLICATION env variable not found, it is not set in non-bluemix env');
            }
            ...
            if(process.env.APM_BM_SECURE_GATEWAY || process.env.APM_BM_GATEWAY_URL || process.env.MONITORING_SERVER_URL
            ...
            if (!this.mAndA) {
                logger.info('JsonSender: Neither MonitoringAndAnalytics service nor APM_BM_SECURE_GATEWAY was found');
            }
            
            if (process.env.VCAP_APPLICATION || fs.existsSync(__dirname + '/../etc/global.environment')) {
                //console.info('VCAP_APPLICATION: ',process.env.VCAP_APPLICATION);
                if(process.env.VCAP_APPLICATION){
                    var vcapApplication = JSON.parse(process.env.VCAP_APPLICATION);       
                    ...
                    if(!process.env.MONITORING_APPLICATION_NAME){
                        ...
                        if(process.mainModule.paths && process.mainModule.paths.length > 0){
                            ...
                            if(!name){
                                logger.warn('Failed to get name in package.json, will generate applicationName and APP_GUID by file position.');
                        ...
                        this.app_guid = this.app_hostname+'_'+process.env.MONITORING_APPLICATION_NAME;
                    }
                    this.app_data = {
                ...
                logger.error('JsonSender: VCAP_APPLICATION env variable not found and there is no global.environment in etc folder');
            }
        }
                        ...
                        if( typeof service_inst[k] == 'string'){
                            ...
                            for( var c_key in credentials){
                                if( credential_exclude_keys.indexOf(c_key) < 0 ) {
                                    external_obj.properties['credentials.'+c_key] = credentials[c_key];
                                }
                            }
                        }
                    }
        ...
        if(/*getServerPort() != 'unknown' && */!this.registeredAll){
            ...
            var nodeappPayload = {
                    ...
                    references:[
                        ...
                        {direction:'from', type:'manages', id: this.dcMD5String, nodetype: "provider"}
                    ]
            };
...
JsonSender.prototype.send = function send(data) { //data = {appInfo:{}, httpReq:[], GC:{}, El;{}, profiling:{}}
    
    if(data == undefined || data.appInfo == undefined){
        return;
    }
    ...
    if (this.mAndA) {
        ...
        if(data.appInfo.PORT == 'unknown' || data.appInfo.PORT == undefined){
        ...
        var data = this.merge([initData, this.app_data, data.appInfo, /*data.GC, data.El, */reqData]);
        ...
        var post_data = JSON.stringify(data, convertTypes);
    ...
    //Send to APM_BM_Gateway
        if(process.env.APM_BM_SECURE_GATEWAY || ((process.env.APM_BM_GATEWAY_URL || process.env.MONITORING_SERVER_URL) && (senderTool.BM_info.done)) ){
            ...
            if(!process.env.HYBRID_BMAPPID || process.env.HYBRID_BMAPPID == 'undefined'){
                    //var urlString = "https://" + process.env.APM_BM_SECURE_GATEWAY + "/OEReceiver/getMSNs/" + this.app_guid;
                    BM_options.method = 'GET';
                    BM_options.headers = undefined;
                    if(process.env.APM_GW_PROXY_CONNECTION && isHttp){
                        //BM_options.path = "http://"+urlMap['hostname']+":"+(urlMap['port']?urlMap['port']:BM_info.port)+"/OEReceiver/getMSNs/" + this.app_guid;
                    ...
                    if(process.env.APM_GW_PROXY_CONNECTION && isHttp && senderTool.httpProxy){
                        try{
                            senderTool.httpProxy.get('http://'+BM_options.hostname+':'+BM_options.port+BM_options.path, function (err, resp, body) {
                                if(err){
                                    logger.error(err);
                                }
                                if(body){
                                    try{
                                        process.env.HYBRID_BMAPPID = JSON.parse(body.toString())['MSNs'][0];
...
JsonSender.prototype.sendAAR = function(req_inst){
    ...
    if(!(testTrue(process.env.KNJ_ENABLE_TT)) && process.env.KNJ_ENVTYPE == 'Cloudnative') {    // send AAR from http request at resource level

        var interaction_info = {};
        if(req_inst.header){
            interaction_info = aarTools.extractInfoFromHeader(req_inst.header,req_inst.requestHeader);
        }
    ...
    }else if(process.env.KNJ_ENVTYPE == 'Cloudnative' || (testTrue(process.env.KNJ_ENABLE_TT) && process.env.KNJ_ENVTYPE == 'CloudOE')){
        this.RequestMapping[req_inst.time] = req_inst;
    }
};

JsonSender.prototype.storeForAAR = function(req_inst){
    if(process.env.KNJ_ENVTYPE == 'Cloudnative' || (testTrue(process.env.KNJ_ENABLE_TT) && process.env.KNJ_ENVTYPE == 'CloudOE')){
        this.RequestMapping[req_inst.time] = req_inst;
    }
}
    ...
    if(process.env.KNJ_ENVTYPE == 'Cloudnative'){
        ...
        if(process.env.HYBRID_BMAPPID){
            ...
            var BM_options = {
                ...
                host: urlMap['host'],
                port: urlMap['port']?urlMap['port']:BM_info.port,
                path: urlMap['path']+'?tenant=34&origin='+process.env.HYBRID_BMAPPID+'&type=aar/middleware',
                pfx: BM_info.pfx,
                passphrase: BM_info.passphrase,
                cer: BM_info.cert,
                key: BM_info.key,
...
JsonSender.prototype.sendADR = function(data){
    var payload_json = {
        properties:{
            ...
            documentID: uuid.v1(),
            reqType: data.type,
            methodEntries: testTrue(process.env.KNJ_ENABLE_METHODTRACE)?"true":"false",
            reqName: data.name
        },
        statistics:{
            summary:{
