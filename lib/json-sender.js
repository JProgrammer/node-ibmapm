        ...
        // Event loop and GC have multiple event in one minute
            ...
            for (var i in data.GC_Arr){
                enginePayload.push(commonTools.merge([
                    enginePayloadMeta, {
                        timestamp: new Date(data.GC_Arr[i].TIMESTAMP).toISOString(),
                        metrics: {
                            GC_DURATION: data.GC_Arr[i].GC_DURATION,
                            GC_TYPE: data.GC_Arr[i].GC_TYPE,
                            HEAP_USED: data.GC_Arr[i].HEAP_USED,
                            HEAP_SIZE: data.GC_Arr[i].HEAP_SIZE
                        }
                    }]));
            }
            for (var j in data.EL_Arr){
                enginePayload.push(commonTools.merge([
                    enginePayloadMeta, {
                        timestamp: new Date(data.EL_Arr[j].TIMESTAMP).toISOString(),
                        metrics: {
                            EVENTLOOP_LATENCY_AVG: data.EL_Arr[j].EVENTLOOP_LATENCY_AVG,
                            EVENTLOOP_LATENCY_MIN: data.EL_Arr[j].EVENTLOOP_LATENCY_MIN,
                            EVENTLOOP_LATENCY_MAX: data.EL_Arr[j].EVENTLOOP_LATENCY_MAX
                        }
                    }]));
            }
            for (var k in data.Loop_Arr){
                enginePayload.push(commonTools.merge([
                    enginePayloadMeta, {
                        timestamp: new Date(data.Loop_Arr[k].TIMESTAMP).toISOString(),
                        metrics: {
