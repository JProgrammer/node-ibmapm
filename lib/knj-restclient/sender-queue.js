...
function httpSend(task, quequeName, httpurl,payload,type,additionalHeader,addtionalOptions, callback){    //special for cloudnative register
    ...
    if(addtionalOptions){
        for (var key in addtionalOptions){
            options[key] = addtionalOptions[key];
        }
    }
    
    if(process.env.KNJ_PROXY && urlMap.protocol == 'http:'){
            httpProxy = httpProxy?httpProxy:request.defaults({proxy:process.env.KNJ_PROXY,rejectUnauthorized:false});
            try{
                httpProxy.post('http://'+options.hostname+':'+options.port+options.path, {
                    ...
                    if(resp){
                        logger.info("Request "+ type +" through proxy, response statusCode: " + resp.statusCode);
                    }
                });
            }catch(e){
...
senderQueue.prototype.addTask = function(task){
    ...
    }else if(task.type && task.type.indexOf('resources_') == 0){
        this.resourceQueue.push(task);
    }else if(task.type && task.type.indexOf('metrics_') == 0){
        this.metricQueue.push(task);
    }else if(task.type && task.type.indexOf('aar_') == 0){
        this.aaradrQueue.push(task);
    }else if(task.type && task.type.indexOf('adr_') == 0){
        this.aaradrQueue.push(task);
    }else{
...
senderQueue.prototype.consume = function(){ //sequence: dc -> resource -> metric -> aar/adr
    var task;
    ...
    if(task){
        ...
        httpSend(task, this.name, task.url,task.payload,task.type,task.additionalHeader, undefined, task.callback);
    }
}
