...
var cfg = {
    ...
    AAR: 'aar/middleware',
    ADR: 'adr/middleware',
    AMServiceName: 'AvailabilityMonitoring-APD'
};
...
module.exports.setConfiguration = function(fileName, callback) {
    ...
    try {
        ...
        if (!cfg.AMServiceName) {
            cfg.AMServiceName = 'AvailabilityMonitoring-APD';
        }

    } catch (e) {
        logger.error('register_topology set cofniguration failed.', e);
    }
...
module.exports.refreshBAMConfig = function(callback) {
    ...
    if (process.env.VCAP_SERVICES) {
        ...
        if (vcap_service[cfg.AMServiceName]) {
            process.env.AM_SERVICE_BOUND = true;
            var cred_url = vcap_service[cfg.AMServiceName][0].credentials.cred_url + SB_PATH + app_guid;
            var token = vcap_service[cfg.AMServiceName][0].credentials.token;
                ...
                res.on('data', function(d) {
                    try {
                        ...
                        if (rescontent['backend_url'] && rescontent['token']) {
                            ...
                            if (callback) {
                                logger.debug("The backend BAM service is ready, invoke the callback ", callback.name);
                                callback();
                            }
                        }
