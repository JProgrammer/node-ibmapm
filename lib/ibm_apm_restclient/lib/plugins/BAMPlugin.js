...
function sendOne(task, callback) {
    ...
    if (IBAM_options) {
        ...
        if (task.type && task.type.indexOf('amui:') === 0) {
            task.url = wf_url + WF_SVCS_CONN.UI_SERVICE_BASE_PATH + '/bulk' + '?version=' + global.DC_VERSION;
...
function dumpResourceIDs4Test(type, payload) {
    if (process.env.DUMP_PROPERTIES_AVT) {
        ...
        if (type === 'dc' || type.substr(0, 'resources'.length) === 'resources') {
            if (payload.hasOwnProperty('urn') && payload.hasOwnProperty('entityTypes')) {
                let entityTypes = payload.entityTypes;
                if (entityTypes.length > 0) {
                    item = payload.urn + ':' + payload.entityTypes[0] + '\n';
...
function sendMetadata() {
    ...
    try {
        ...
        let mdruntime = require('../../etc/ResourceTypes/nodeApplicationRuntime_1.2_ResourceType.json');
