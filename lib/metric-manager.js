...
MetricManager.prototype.readConfig = function readConfig()  {
    ...
    if( this.metric.port || this.envType === 'CloudOE' || this.envType === 'Cloudnative')  {
        var confFile = __dirname + (this.metric.port ? ('/plugin_' + this.metric.port + '_conf.json') : '/plugin_conf.json');
        ...
        try  {
            ...
            if(e.code == 'ENOENT' && e.path){
                ...
                logger.info('Configuration file will be genrated in one minute, if the agent is started.');
            }else {
        ...
        var self = this;
        if(config){
            fs.watchFile(confFile, {persistent: false, interval:self.interval}, function(pre, curr){
                var config = undefined;
                try  {
                    var confString = fs.readFileSync( confFile, 'utf8' );
                    config = JSON.parse( confString );
...
MetricManager.prototype.start = function(envType)  {
    ...
    if (envType === 'CloudOE' || envType === 'Cloudnative') {   // don't need to wait for plugin_xxx_conf.json
    ...
    this.timer = setInterval( function doThings()  {
        ...
        if( self.metric && self.wake_up%2>>>0 == 0 )  { //collect data
            ...
            metricSender.send( self.metric, (self.envType === 'CloudOE' || self.envType === 'Cloudnative') ? jsonSender : socketSender);
