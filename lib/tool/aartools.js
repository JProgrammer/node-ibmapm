...
function processAARInteractions(req, result, port, context) {
    ...
    if (!req.type) {
        ...
        if (!(req.type === 'HTTP' && req.parent)) {
            ...
            interaction_info.componentName = req.type === 'HTTP' ?
            ...
            if (req_inst) {
                if (req_inst.header) {
                    ...
                    if (req_inst.header) {
                        var header_info = extractInfoFromHeader(req_inst.header,
                            req_inst.requestHeader);
                        ...
                        interaction_info.url = getFullURL(req.name, context.IP, port,
                            header_info.protocol);
                    }
                }
