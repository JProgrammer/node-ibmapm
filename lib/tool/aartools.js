...
function extractInfoFromHeader(header, reqHeader) {
    ...
    /* if (reqHeader && reqHeader.referer) {
        interaction_info.fullurl = reqHeader.referer;
    }*/
...
function processAARInteractions(req, result, port, context) {
    var traceIt = true;
    // TODEL var time = req.timer.startTimeMillis;
    var interaction_info = {
    };
    var stitch_info;
    var correlator;
    if (!req.type) {
        ...
        if (!(req.type === 'HTTP' && req.parent)) {
            // TODEL var req_inst = context.RequestMapping[time];
            ...
            if (req.context) {
                ...
                // TODEL }
                // TODEL delete context.RequestMapping[time];
            }
        ...
        // TODEL interaction_info.query = req.context.query;
...
exports.composeAARTT = function composeAARTT(data, port) {
    ...
    // TODEL var req_inst = context.RequestMapping[data.time];
