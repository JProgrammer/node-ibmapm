...
module.exports.init = function(config) {
    ...
    if(process.env.VCAP_SERVICES){
        ...
        if(env[MAServiceName]){
                ...
                if(!env[MAServiceName][0].credentials.apphost)env[MAServiceName][0].credentials.apphost = "test";
...
module.exports.update = function (config) {
    ...
    //if if using healthcenter
//  if(newConfig.profiling){
//      healthcenter.enable('profiling');
//  }else{
//      healthcenter.disable('profiling');
//  }
    if (testTrue(process.env.KNJ_ENABLE_TT) || testTrue(process.env.KNJ_ENABLE_DEEPDIVE) || process.env.APM_BM_SECURE_GATEWAY 
        ...
        if(testTrue(process.env.KNJ_ENABLE_TT) || testTrue(process.env.KNJ_ENABLE_METHODTRACE)){
...
module.exports.setMethodProbes = function(p){
    ...
    mProbes.includeModules = cfg.deepDive.includeModules = p.include_modules?p.include_modules:[];
    mProbes.excludeModules = cfg.deepDive.excludeModules = p.exclude_modules?p.exclude_modules:[];
